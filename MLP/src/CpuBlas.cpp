...
void CpuBlas::convolve(double *const A, double *const W,
                       double *const C, const int N, const int M, const int K1, const int K2,
                       const double beta) {
    const int P = N - K1 + 1;
    const int Q = M - K2 + 1;
    ...
    for (int i = 0; i < P; i++) {
        for (int j = 0; j < Q; j++) {
            ...
            C[i*Q + j] = cblas_ddot(K1*K2, tmp.data(), 1, W, 1);
        }
    }
}
