...
TEST(Conv2d, fast_forward) {
    ...
    Conv2d conv2D(2, 2, 3, make_unique<CpuBlas>(), w, true);
    Tensor input(a, {1, 2, 6, 5});
    Tensor output = conv2D.forward(Tensor(input));
    Tensor in_grad = conv2D.backward(Tensor(std::vector<double>(24, 1.), {1, 2, 4, 3}));

    std::vector<double> correct_out{34.0, 290.0, 107.0, 33.0, 341.0, 112.0, 78.0, 394.0, 121.0, 137.0, 425.0, 163.0,
                                    353.0, 342.0, 407.0, 433.0, 389.0, 430.0, 538.0, 465.0, 471.0, 659.0, 549.0, 528.0};

    vector<double> correct_in_grad{};
    vector<double> grad{15.0, 18.0, 25.0, 21.0, 23.0, 28.0, 30.0, 31.0, 34.0, 36.0, 39.0, 46.0, 44.0, 46.0, 51.0, 44.0,
                        45.0, 48.0, 15.0, 18.0, 25.0, 21.0, 23.0, 28.0, 30.0, 31.0, 34.0, 36.0, 39.0, 46.0, 44.0, 46.0,
                        51.0, 44.0, 45.0, 48.0,
                        12, 12};

    ASSERT_EQ(is_same_vectors(output.data(), correct_out, 1e-5), true);
    //ASSERT_EQ(is_same_vectors(in_grad.data(), correct_out, 1e-5), true);
    ASSERT_EQ(is_same_vectors(conv2D.getParametersGradient(), grad, 1e-5), true);
}
