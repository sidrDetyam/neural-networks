...
Conv2dNaive::Conv2dNaive(const size_t input_channels,
...
Tensor Conv2dNaive::forward(Tensor &&input) {
...
Tensor Conv2dNaive::backward(const Tensor &output) {
...
std::vector<double> &Conv2dNaive::getParametersGradient() {
    return grad_.data();
}

std::vector<double> &Conv2dNaive::getParameters() {
    return params_.data();
}

std::vector<size_t> Conv2dNaive::get_output_shape(const std::vector<size_t> &input_shape) const {
